...
- Event Debugger window (Tools ▸ Event Debugger): live editor-only trace of publishes and deliveries per bus, with chain-depth indentation, per-instance filtering, and `EventBus.SetDebugName` for readable instance names. No instrumentation is compiled into player builds.
